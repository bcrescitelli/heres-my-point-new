      ...
      <div className="min-h-screen bg-indigo-950 text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden">
        <div className="max-w-md w-full text-center space-y-12 animate-in fade-in zoom-in duration-500">
          ...
          <div className="grid gap-6">
            <button onClick={createRoom} className="group relative bg-indigo-600 hover:bg-indigo-500 p-8 rounded-[2.5rem] flex items-center justify-between border-b-8 border-indigo-900 shadow-2xl transition-all active:translate-y-2 active:border-b-0"><div className="text-left relative z-10"><p className="font-black text-2xl uppercase italic leading-none mb-1 text-white">Host Show</p><p className="text-indigo-300 text-sm font-bold">Project on Big Screen</p></div><Monitor className="w-12 h-12 text-indigo-300" /></button>
            <div className="bg-indigo-900/40 p-8 rounded-[2.5rem] border-2 border-indigo-800 space-y-6 text-left shadow-xl">
              ...
              <form onSubmit={joinRoom} className="space-y-4 relative z-10">
                <input type="text" placeholder="CODE" maxLength={4} className="w-full bg-indigo-950 text-center font-black text-5xl py-4 rounded-3xl uppercase text-white border-2 border-indigo-800 shadow-inner focus:ring-4 focus:ring-yellow-400" value={roomCode} onChange={e => setRoomCode(e.target.value)} />
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative text-center overflow-hidden">
        ...
        {room.status === 'TOPIC_REVEAL' && (
          <div className="space-y-12 animate-in zoom-in duration-500 w-full max-w-5xl overflow-hidden">
            ...
            <div className="flex flex-col items-center gap-6 leading-none">
               {room.roundType === 2 && !room.sneakyWord ? (<div className="bg-emerald-500/10 border-2 border-emerald-500/30 p-8 rounded-[2rem] animate-pulse leading-none"><p className="text-emerald-400 font-black uppercase text-xl italic tracking-tighter leading-none">The Plant is Sabotaging...</p></div>) : (<div className="relative group leading-none"><div className="absolute inset-0 bg-yellow-400 blur-2xl opacity-10 animate-pulse leading-none"></div><div className="relative bg-indigo-900/50 px-12 py-8 rounded-[3rem] border-2 border-indigo-700 flex flex-col items-center leading-none"><p className="text-indigo-400 font-black uppercase text-[10px] mb-2 tracking-[0.3em] leading-none">Preparation Clock</p><p className="text-8xl font-black text-white tabular-nums drop-shadow-glow leading-none">{room.prepCountdown}</p></div></div>)}
            </div>
          </div>
        )}
        ...
        {room.status === 'VOTING_VIBE' && (
          <div className="space-y-12 animate-in zoom-in w-full max-w-5xl leading-none">
            ...
            <div className="flex justify-center gap-6 md:gap-12 flex-wrap leading-none">
              <div className="bg-indigo-900/50 p-10 md:p-12 rounded-[3rem] border-4 border-indigo-700 min-w-[280px] md:min-w-[350px] leading-none"><p className="text-7xl md:text-9xl font-black text-white tabular-nums leading-none">{Object.values(room.votes || {}).filter(v => v === speaker?.uid).length}</p><p className="text-2xl md:text-3xl font-black mt-6 uppercase text-indigo-300 leading-none">{speaker?.name}</p></div>
              <div className="bg-indigo-900/50 p-10 md:p-12 rounded-[3rem] border-4 border-indigo-700 min-w-[280px] md:min-w-[350px] leading-none"><p className="text-7xl md:text-9xl font-black text-white tabular-nums leading-none">{Object.values(room.votes || {}).filter(v => v === opponent?.uid).length}</p><p className="text-2xl md:text-3xl font-black mt-6 uppercase text-indigo-300 leading-none">{opponent?.name}</p></div>
            </div>
          </div>
        )}
        ...
        {room.status === 'FINAL_PODIUM' && (
           <div className="space-y-12 animate-in slide-in-from-bottom-20 duration-1000 w-full max-w-4xl px-4 overflow-hidden relative text-center leading-none">
              ...
              <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter text-white leading-none leading-none">The Winners</h2>
              <div className="grid gap-4 mt-12 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide leading-none">{sortedWinners.slice(0, 3).map((p, i) => (<div key={p.uid} className={`flex items-center justify-between p-6 md:p-8 rounded-[2.5rem] border-4 backdrop-blur-md shadow-2xl ${i === 0 ? 'bg-yellow-400 text-indigo-950 border-white/50 scale-105' : 'bg-indigo-900 text-white border-indigo-700 opacity-80'} leading-none`}><div className="flex items-center gap-6 leading-none leading-none"><span className="text-4xl md:text-6xl font-black italic opacity-40 leading-none">#{i+1}</span><span className="text-3xl md:text-5xl font-black uppercase tracking-tighter truncate max-w-[200px] leading-none">{p.name}</span></div><div className="text-right leading-none leading-none"><span className="text-3xl md:text-5xl font-black leading-none">{p.score}</span><span className="text-[10px] uppercase font-black block tracking-widest leading-none">Points</span></div></div>))}</div>
              <button onClick={restartGame} className="bg-indigo-600 px-12 py-5 rounded-2xl font-black text-xl md:text-2xl uppercase flex items-center gap-3 shadow-xl active:scale-95 mx-auto mt-8 leading-none uppercase leading-none"><RotateCcw className="w-6 h-6 leading-none" /> Replay Show</button>
           </div>
        )}
      </div>

      <div className="bg-indigo-900 p-6 md:p-8 flex justify-center flex-wrap gap-4 md:gap-8 border-t-4 border-black/20 shadow-2xl max-h-[140px] overflow-hidden shrink-0 leading-none">
        {players.sort((a,b) => b.score - a.score).map((p, i) => (<div key={p.uid} className="flex items-center gap-4 bg-indigo-950/50 px-5 md:px-6 py-3 rounded-2xl border-2 border-indigo-700 shadow-inner group transition-all leading-none"><div className="font-black text-indigo-500 text-2xl italic group-hover:text-yellow-400 leading-none">#{i+1}</div><div className="text-left leading-tight leading-none"><p className="font-black uppercase text-[10px] md:text-xs tracking-tighter truncate max-w-[80px] text-indigo-100 uppercase leading-none">{p.name}</p><p className="font-black text-yellow-400 text-xl md:text-2xl leading-none mt-1 leading-none">{p.score}</p></div></div>))}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-[100dvh] bg-indigo-950 text-white flex flex-col font-sans touch-none select-none overflow-hidden max-w-full uppercase leading-none">
       ...
       <div className="flex-1 flex flex-col p-6 overflow-y-auto leading-none">
          ...
          {room.status === 'ROUND_INTRO' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-8 text-center animate-in zoom-in leading-none">
              <div className="bg-indigo-900 p-8 rounded-[2.5rem] border-2 border-indigo-800 shadow-xl w-full leading-none">
                ...
                <h3 className="text-4xl font-black italic mb-3 text-yellow-400 uppercase tracking-tighter leading-none leading-none">{ROUND_DETAILS[room.roundType].title}</h3>
              ...
              {isLeader && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-8 rounded-[2rem] font-black text-2xl shadow-xl active:scale-95 leading-none uppercase leading-none">PICK PROMPT</button>}
            </div>
          )}
          ...
          {room.status === 'VOTING_VIBE' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none leading-none">
               <h2 className="text-4xl font-black uppercase text-pink-500 italic tracking-tighter leading-none text-center leading-none leading-none">Vibe Check!</h2>
               <button onClick={() => castVote(room.currentSpeakerUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none leading-none">{players.find(p => p.uid === room.currentSpeakerUid)?.name}</button>
               <button onClick={() => castVote(room.opponentUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none leading-none">{players.find(p => p.uid === room.opponentUid)?.name}</button>
               {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 mt-4 leading-none uppercase leading-none">Award Bonus</button>}
            </div>
          )}

          {room.status === 'RESULTS' && <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in zoom-in leading-none leading-none"><Trophy className="w-24 h-24 text-yellow-400 animate-bounce leading-none" /><h2 className="text-5xl font-black italic tracking-tighter text-center leading-none leading-none leading-none">Turn<br/>Finished</h2>{(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-yellow-400 text-indigo-950 py-10 rounded-[3rem] font-black text-4xl shadow-2xl active:scale-95 animate-pulse leading-none uppercase leading-none">Continue Show</button>}</div >}
       </div>
       <style>{`

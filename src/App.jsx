...
const ROUND_DETAILS = {
  1: {
    title: "Solo Standard",
    description: "Defend your prompt for 30s. Don't look at the timer!",
    icon: <Mic2 className="w-16 h-16 text-yellow-400" />
  },
  2: {
    title: "The Sneaky Word",
    description: "Use a secret 'Planted' word naturally for a 300pt bonus.",
    icon: <MessageSquare className="w-16 h-16 text-emerald-400" />
  },
  3: {
    title: "The Face-Off",
    description: "Two players. One topic. At 15s, SWITCH and defend the other side!",
...
export default function App() {
      ...
      if (snap.exists()) {
        ...
        if (data.lastHeckle?.id && data.lastHeckle.id !== lastHeckleIdRef.current) {
          ...
          if (role === 'host') {
            ...
            new Audio(`/sounds/sound${idx}.${AUDIO_EXT}`).play().catch(() => {});
          }
  ...
  const setupTurn = async () => {
    ...
    // Prompt Selection Logic (Used Prompts Tracking)
    const used = room.usedTopics || [];
    const available = TOPICS.filter(t => !used.includes(t));
    const topic = available.length > 0 
      ? available[Math.floor(Math.random() * available.length)] 
      : TOPICS[Math.floor(Math.random() * TOPICS.length)]; // Fallback if all 120+ used
  ...
  if (authLoading) return <div className="min-h-screen bg-indigo-950 flex flex-col items-center justify-center p-8 text-white uppercase"><Loader2 className="animate-spin mb-4" /><p className="uppercase text-xs font-black tracking-widest">Waking Up...</p></div>;
      ...
      <div className="min-h-screen bg-indigo-950 text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden">
        <div className="max-w-md w-full text-center space-y-12 animate-in fade-in zoom-in duration-500">
          ...
          <div className="grid gap-6">
            <button onClick={createRoom} className="group relative bg-indigo-600 hover:bg-indigo-500 p-8 rounded-[2.5rem] flex items-center justify-between border-b-8 border-indigo-900 transition-all active:translate-y-2 active:border-b-0 shadow-2xl overflow-hidden"><div className="text-left relative z-10"><p className="font-black text-2xl uppercase italic leading-none mb-1 text-white uppercase">Host Show</p><p className="text-indigo-300 text-sm font-bold">Project on the Big Screen</p></div><Monitor className="w-12 h-12 text-indigo-300 group-hover:text-yellow-400 transition-colors relative z-10" /></button>
  ...
  return role === 'host' ? (
    <HostView room={room} players={players} roomCode={roomCode} startSpeaking={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', roomCode), { status: 'SPEAKING', startTime: Date.now() })} activeHeckle={activeHeckle} restartGame={restartGame} />
  ) : (
    <PlayerView room={room} players={players} user={user} stopSpeaking={stopSpeaking} setupTurn={setupTurn} advanceGame={advanceGame} startNextRound={startNextRound} />
  );
}
    ...
    <div className="min-h-screen bg-indigo-950 text-white flex flex-col font-sans overflow-hidden select-none uppercase">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative text-center overflow-hidden">
        {room.status === 'LOBBY' && (
          <div className="space-y-12 max-w-4xl animate-in zoom-in w-full">
             ...
             {players.length >= 2 && <p className="text-yellow-400 font-black animate-pulse tracking-widest">Leader controls the show...</p>}
          </div>
        )}
        ...
        {room.status === 'ROUND_INTRO' && (
          <div className="space-y-8 animate-in slide-in-from-bottom duration-700 max-w-3xl w-full">
            ...
            <h2 className="text-6xl md:text-8xl font-black italic text-white leading-none leading-none">{ROUND_DETAILS[room.roundType].title}</h2>
        ...
        {room.status === 'TOPIC_REVEAL' && (
          <div className="space-y-12 animate-in zoom-in w-full max-w-5xl">
            <div className="space-y-4"><p className="text-yellow-400 font-black uppercase tracking-[0.3em] text-xs italic leading-none">The Controversy</p><h2 className="text-5xl md:text-8xl font-black italic leading-tight text-white drop-shadow-lg break-words leading-none">"{room.topic}"</h2></div>
            <div className="flex justify-center gap-8 md:gap-16 items-center flex-wrap">
              ...
              {room.roundType === 3 && (<><Swords className="w-8 h-8 md:w-12 md:h-12 text-pink-500 animate-pulse leading-none" /><div className="space-y-2"><p className="text-indigo-400 uppercase font-black text-[10px] tracking-widest leading-none">Opponent</p><div className="text-3xl md:text-5xl font-black text-white uppercase leading-none">{opponent?.name}</div></div></>)}
            </div>
            <div className="flex flex-col items-center gap-6">
               {room.roundType === 2 && !room.sneakyWord ? (<div className="bg-emerald-500/10 border-2 border-emerald-500/30 p-8 rounded-[2rem] animate-pulse leading-none"><p className="text-emerald-400 font-black uppercase text-xl italic leading-none uppercase">The Plant is Sabotaging...</p></div>) : (<div className="relative group leading-none"><div className="absolute inset-0 bg-yellow-400 blur-2xl opacity-10 animate-pulse leading-none"></div><div className="relative bg-indigo-900/50 px-12 py-8 rounded-[3rem] border-2 border-indigo-700 flex flex-col items-center leading-none"><p className="text-indigo-400 font-black uppercase text-[10px] mb-2 tracking-[0.3em] leading-none">Preparation Clock</p><p className="text-8xl font-black text-white tabular-nums drop-shadow-glow leading-none">{room.prepCountdown}</p></div></div>)}
            </div>
          </div>
        )}

        {room.status === 'SPEAKING' && (
          <div className="space-y-12 w-full max-w-6xl animate-in fade-in">
            ...
            <div className="flex flex-col items-center gap-12">
              <div className="relative w-56 h-56 md:w-72 md:h-72 flex items-center justify-center"><div className="absolute inset-0 border-8 border-indigo-800 rounded-full"></div><div className={`absolute inset-0 border-8 border-yellow-400 rounded-full animate-ping opacity-20`} style={{ animationDuration: '2s' }}></div><Mic2 className="w-16 h-16 md:w-24 md:h-24 text-yellow-400 drop-shadow-glow" /></div>
        ...
        {activeHeckle && (
          <div key={activeHeckle.id} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-in zoom-in slide-in-from-bottom-20 pointer-events-none z-50 w-full max-w-2xl text-center">
            ...
            <p className="mt-6 text-2xl font-bold bg-indigo-950 px-6 py-3 rounded-2xl border-2 border-white/20 shadow-xl inline-block">{activeHeckle.sender}</p>
          </div>
        )}
        ...
        {room.status === 'RESULTS' && (
          <div className="space-y-12 animate-in zoom-in w-full overflow-hidden text-center leading-none">
             ...
             <p className="text-2xl font-black uppercase text-yellow-400 italic animate-pulse leading-none mt-8">Speaker controls the show...</p>
          </div>
        )}

        {room.status === 'FINAL_PODIUM' && (
           <div className="space-y-12 animate-in slide-in-from-bottom-20 duration-1000 w-full max-w-4xl px-4 overflow-hidden relative text-center leading-none">
              <Confetti />
              <Trophy className="w-32 h-32 md:w-48 md:h-48 text-yellow-400 mx-auto animate-bounce relative z-10 drop-shadow-glow leading-none" />
              <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter text-white leading-none">The Winners</h2>
              <div className="grid gap-4 mt-12 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">{sortedWinners.slice(0, 3).map((p, i) => (<div key={p.uid} className={`flex items-center justify-between p-6 md:p-8 rounded-[2.5rem] border-4 backdrop-blur-md shadow-2xl ${i === 0 ? 'bg-yellow-400 text-indigo-950 border-white/50 scale-105' : 'bg-indigo-900 text-white border-indigo-700 opacity-80'} leading-none`}><div className="flex items-center gap-6 leading-none"><span className="text-4xl md:text-6xl font-black italic opacity-40 leading-none">#{i+1}</span><span className="text-3xl md:text-5xl font-black uppercase tracking-tighter truncate max-w-[200px] leading-none">{p.name}</span></div><div className="text-right leading-none"><span className="text-3xl md:text-5xl font-black leading-none">{p.score}</span><span className="text-[10px] uppercase font-black block tracking-widest leading-none">Points</span></div></div>))}</div>
              <button onClick={restartGame} className="bg-indigo-600 px-12 py-5 rounded-2xl font-black text-xl md:text-2xl uppercase flex items-center gap-3 shadow-xl active:scale-95 mx-auto mt-8 leading-none"><RotateCcw className="w-6 h-6 leading-none" /> Replay Show</button>
           </div>
        )}
      </div>
      ...
      {room.status !== 'FINAL_PODIUM' && room.status !== 'LOBBY' && (
        <div className="bg-indigo-900 p-6 md:p-8 flex justify-center flex-wrap gap-4 md:gap-8 border-t-4 border-black/20 shadow-2xl max-h-[140px] overflow-hidden shrink-0">
          {players.sort((a,b) => b.score - a.score).map((p, i) => (<div key={p.uid} className="flex items-center gap-4 bg-indigo-950/50 px-5 md:px-6 py-3 rounded-2xl border-2 border-indigo-700 shadow-inner group transition-all leading-none"><div className="font-black text-indigo-500 text-2xl italic group-hover:text-yellow-400 leading-none">#{i+1}</div><div className="text-left leading-tight leading-none"><p className="font-black uppercase text-[10px] md:text-xs tracking-tighter truncate max-w-[80px] text-indigo-100 leading-none">{p.name}</p><p className="font-black text-yellow-400 text-xl md:text-2xl leading-none mt-1 leading-none">{p.score}</p></div></div>))}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-[100dvh] bg-indigo-950 text-white flex flex-col font-sans touch-none select-none overflow-hidden max-w-full uppercase leading-none">
       <div className="p-4 bg-indigo-900 flex justify-between items-center border-b-2 border-black/20 shadow-md shrink-0 leading-none">
          <div className="flex items-center gap-3 leading-none"><div className="w-10 h-10 rounded-full bg-yellow-400 flex items-center justify-center text-indigo-950 font-black leading-none">{me?.name?.charAt(0) || '?'}</div><span className="font-black text-sm truncate max-w-[100px] leading-none uppercase">{me?.name}</span></div>
          <div className="flex gap-4 leading-none">
             <div className="text-right leading-none border-r border-white/10 pr-4 leading-none"><p className="text-[10px] font-black text-indigo-400 mb-1 leading-none uppercase leading-none uppercase">Ammo</p><div className="flex gap-0.5 leading-none">{[...Array(MAX_HECKLES)].map((_, i) => (<div key={i} className={`w-1 h-2 rounded-full ${i < (me?.hecklesLeft || 0) ? 'bg-red-500' : 'bg-indigo-950 opacity-30'} leading-none`} />))}</div></div>
             <div className="text-right leading-none leading-none"><p className="text-[10px] font-black text-indigo-400 mb-1 leading-none uppercase leading-none uppercase">Score</p><p className="text-lg font-black text-yellow-400 tabular-nums leading-none leading-none">{me?.score || 0}</p></div>
          </div>
       </div>

       <div className="flex-1 flex flex-col p-6 overflow-y-auto leading-none">
          {room.status === 'LOBBY' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 leading-none">
              <Users className="w-20 h-20 text-indigo-400 animate-pulse leading-none" />
              <h2 className="text-4xl font-black uppercase italic tracking-tighter text-white leading-none uppercase">HERE'S MY POINT!</h2>
          ...
          {room.status === 'ROUND_INTRO' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-8 text-center animate-in zoom-in leading-none">
              <div className="bg-indigo-900 p-8 rounded-[2.5rem] border-2 border-indigo-800 shadow-xl w-full leading-none">
                <div className="flex justify-center mb-6 scale-75 leading-none">{ROUND_DETAILS[room.roundType].icon}</div>
                <h3 className="text-4xl font-black italic mb-3 text-yellow-400 uppercase tracking-tighter leading-none leading-none uppercase">{ROUND_DETAILS[room.roundType].title}</h3>
                <p className="text-indigo-200 text-xs leading-relaxed opacity-70 italic leading-none uppercase leading-none">Rules are on the big screen!</p>
              </div>
              {isLeader && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-8 rounded-[2rem] font-black text-2xl shadow-xl active:scale-95 leading-none uppercase leading-none uppercase">PICK PROMPT</button>}
            </div>
          )}
          ...
          {room.status === 'TOPIC_REVEAL' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-10 text-center max-w-full leading-none">
              {(isSpeaker || (isOpponent && room.roundType === 3)) ? (
                <>
                  <div className="bg-yellow-400 text-indigo-950 px-8 py-2 rounded-full font-black uppercase text-[10px] tracking-widest animate-bounce leading-none uppercase uppercase">Your Turn!</div>
                  ...
                  {isSpeaker && <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { prepCountdown: 0 })} className="w-full bg-white text-indigo-950 py-10 rounded-[2.5rem] font-black text-4xl uppercase shadow-xl tracking-tighter active:scale-95 transition-all leading-none uppercase uppercase">READY!</button>}
                ...
                <>
                  <h2 className="text-4xl font-black uppercase italic tracking-tighter text-emerald-400 leading-none leading-none uppercase">You are<br/>The Plant!</h2>
                  ...
                  <button onClick={() => { if (!sneakyInput) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { sneakyWord: sneakyInput.toUpperCase() }); setSneakyInput(''); }} className="w-full bg-emerald-500 text-indigo-950 py-6 rounded-[2rem] font-black text-2xl uppercase shadow-lg active:scale-95 leading-none uppercase leading-none uppercase">Sabotage!</button>
                </>
              ) : <div className="bg-indigo-900/50 p-10 rounded-[2rem] border border-white/5 w-full text-center leading-none leading-none uppercase"><p className="text-xl font-black uppercase text-indigo-400 tracking-widest animate-pulse leading-none uppercase leading-none">Wait for Setup...</p></div>}
            </div>
          )}
            ...
            <div className="flex-1 flex flex-col max-w-full leading-none">
              {(isSpeaker || isOpponent) ? (
                <div className="flex-1 flex flex-col items-center justify-center space-y-12 leading-none">
                   <div className="text-center space-y-2 leading-none leading-none uppercase"><p className="text-yellow-400 font-black uppercase text-[10px] tracking-widest leading-none uppercase uppercase">Mic is Active</p><h2 className="text-5xl font-black italic uppercase tracking-tighter leading-none leading-none">{(isOpponent && !room.isSecondHalf) ? 'ON DECK...' : (isSpeaker && room.isSecondHalf) ? 'WAIT...' : 'DEFEND!'}</h2></div>
                   <div className="w-64 h-64 rounded-full border-[16px] border-indigo-900 flex items-center justify-center relative bg-indigo-950 shadow-inner leading-none leading-none uppercase">{(isOpponent && room.isSecondHalf) || (isSpeaker && !room.isSecondHalf) ? <div className="absolute inset-0 rounded-full border-8 border-yellow-400 animate-ping opacity-10 leading-none leading-none"></div> : null}<Mic2 className={`w-20 h-20 text-yellow-400 ${((isSpeaker && !room.isSecondHalf) || (isOpponent && room.isSecondHalf)) ? 'opacity-100 animate-pulse' : 'opacity-10'} leading-none`} /></div>
                   {((room.roundType !== 3 && isSpeaker) || (room.roundType === 3 && isOpponent && room.isSecondHalf)) && (<button onClick={() => stopSpeaking((Date.now() - room.startTime) / 1000)} className="w-full bg-red-500 py-12 rounded-[3rem] font-black text-5xl uppercase tracking-tighter shadow-[0_12px_0_rgb(150,0,0)] active:translate-y-3 active:shadow-none transition-all leading-none uppercase leading-none uppercase">STOP!</button>)}
                </div>
              ) : (
                <div className="flex-1 flex flex-col space-y-6 max-w-full overflow-hidden leading-none leading-none uppercase">
                   <div className="bg-indigo-900/50 p-4 rounded-[2rem] border border-white/5 text-center leading-none leading-none uppercase"><p className="text-[10px] font-black text-indigo-400 tracking-widest mb-1 leading-none uppercase uppercase">Currently Active</p><h3 className="text-xs font-black italic truncate leading-none opacity-50 leading-none leading-none">"{room.topic}"</h3></div>
                   <form onSubmit={e => { e.preventDefault(); if (!customHeckle || (me?.hecklesLeft || 0) <= 0) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code, 'players', user.uid), { hecklesLeft: increment(-1) }); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { lastHeckle: { id: Math.random().toString(), text: customHeckle.toUpperCase(), sender: me.name } }); setCustomHeckle(''); }} className="space-y-4 leading-none leading-none uppercase">
                      <div className="text-center space-y-2 leading-none leading-none uppercase"><p className="text-[10px] font-black text-red-500 tracking-widest leading-none leading-none uppercase uppercase">Distraction Engine</p><input type="text" placeholder="TYPE A HECKLE..." maxLength={HECKLE_CHAR_LIMIT} className="w-full bg-indigo-900 p-6 rounded-[2rem] border-4 border-indigo-800 font-black text-2xl text-center focus:border-red-500 text-white outline-none leading-none leading-none" value={customHeckle} onChange={e => setCustomHeckle(e.target.value)} /></div>
                      <button type="submit" disabled={(me?.hecklesLeft || 0) <= 0 || !customHeckle} className="w-full bg-red-500 text-white p-7 rounded-[2rem] border-b-8 border-red-900 font-black uppercase text-3xl flex items-center justify-center gap-4 active:translate-y-2 active:border-b-0 shadow-xl disabled:opacity-10 leading-none leading-none uppercase uppercase">FIRE!</button>
                   </form>
                   <div className="mt-auto flex justify-center items-center gap-1 leading-none leading-none uppercase"><p className="text-[10px] font-black text-indigo-400 tracking-widest mr-1 leading-none uppercase leading-none">Ammo:</p>{[...Array(MAX_HECKLES)].map((_, i) => (<div key={i} className={`w-2 h-4 rounded-full transition-all duration-300 ${i < (me?.hecklesLeft || 0) ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-indigo-950 opacity-20'} leading-none`} />))}</div>
                </div>
              )}
            </div>
          )}
          ...
          {room.status === 'VOTING_WORD' && (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none leading-none uppercase">
                <h2 className="text-4xl font-black uppercase text-emerald-400 italic tracking-tighter leading-none leading-none uppercase">Word Check!</h2>
                <div className="bg-indigo-900 p-8 rounded-3xl border-2 border-emerald-500/30 w-full text-center font-black text-3xl text-white italic leading-none uppercase leading-none uppercase">"{room.sneakyWord}"</div>
                <div className="grid grid-cols-2 gap-4 w-full leading-none uppercase"><button onClick={() => castVote('YES')} className="bg-emerald-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none leading-none uppercase"><ThumbsUp className="w-12 h-12 mx-auto leading-none" /></button><button onClick={() => castVote('NO')} className="bg-red-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none leading-none uppercase"><ThumbsDown className="w-12 h-12 mx-auto" /></button></div>
                {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 leading-none uppercase leading-none uppercase leading-none">TALLY VOTES</button>}
             </div>
          )}

          {room.status === 'VOTING_VIBE' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none leading-none uppercase">
               <h2 className="text-4xl font-black uppercase text-pink-500 italic tracking-tighter leading-none text-center leading-none leading-none uppercase">Vibe Check!</h2>
               <button onClick={() => castVote(room.currentSpeakerUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none leading-none uppercase">{players.find(p => p.uid === room.currentSpeakerUid)?.name}</button>
               <button onClick={() => castVote(room.opponentUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none leading-none uppercase">{players.find(p => p.uid === room.opponentUid)?.name}</button>
               {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 mt-4 leading-none uppercase leading-none uppercase leading-none">Award Bonus</button>}
            </div>
          )}

          {room.status === 'RESULTS' && <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in zoom-in leading-none leading-none uppercase"><Trophy className="w-24 h-24 text-yellow-400 animate-bounce leading-none" /><h2 className="text-5xl font-black italic tracking-tighter text-center leading-none leading-none leading-none uppercase">Turn<br/>Finished</h2>{(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-yellow-400 text-indigo-950 py-10 rounded-[3rem] font-black text-4xl shadow-2xl active:scale-95 animate-pulse leading-none uppercase leading-none uppercase">Continue Show</button>}</div >}
       </div>
       <style>{`

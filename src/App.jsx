...
export default function App() {
  ...
  // Robust Audio Player with Fallback and 3-second Auto-Stop
    ...
    return (
      <div className="min-h-screen bg-indigo-950 text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden uppercase">
        <div className="max-w-md w-full text-center space-y-12 animate-in fade-in zoom-in duration-500">
          <div className="space-y-4 leading-none"><h1 className="text-6xl md:text-7xl font-black italic text-yellow-400 uppercase tracking-tighter transform -rotate-2 leading-none">HERE'S MY POINT!</h1><p className="text-indigo-400 font-bold uppercase text-xs tracking-[0.4em] leading-none">The Ultimate Debate Party</p></div>
          <div className="grid gap-6">
            <button onClick={createRoom} className="group relative bg-indigo-600 hover:bg-indigo-500 p-8 rounded-[2.5rem] flex items-center justify-between border-b-8 border-indigo-900 shadow-2xl transition-all active:translate-y-2 active:border-b-0"><div className="text-left"><p className="font-black text-2xl uppercase italic leading-none mb-1 text-white uppercase">Host Show</p><p className="text-indigo-300 text-sm font-bold leading-none">Project on Big Screen</p></div><Monitor className="w-12 h-12 text-indigo-300" /></button>
            <div className="bg-indigo-900/40 p-8 rounded-[2.5rem] border-2 border-indigo-800 space-y-6 text-left shadow-xl uppercase">
              <div className="flex items-center gap-2 text-indigo-300 relative z-10"><Smartphone className="w-5 h-5" /><p className="font-black uppercase text-[10px] tracking-[0.2em] uppercase leading-none">Join the Party</p></div>
              <form onSubmit={e => joinRoom(e)} className="space-y-4">
                ...
                <input type="text" placeholder="YOUR NAME" className="w-full bg-indigo-900 p-5 rounded-2xl font-bold border-2 border-indigo-800 focus:border-indigo-400 text-indigo-100 uppercase leading-none" value={playerName} onChange={e => setPlayerName(e.target.value)} />
...
function HostView({ room, players, roomCode, startSpeaking, activeHeckle, restartGame }) {
  ...
  return (
    <div className="min-h-screen bg-indigo-950 text-white flex flex-col font-sans overflow-hidden select-none uppercase leading-none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative text-center overflow-hidden">
        ...
        {room.status === 'ROUND_INTRO' && (
          <div className="space-y-8 animate-in slide-in-from-bottom duration-700 max-w-3xl w-full leading-none">
            ...
            <p className="text-yellow-400 font-black animate-pulse mt-8 leading-none uppercase uppercase">Waiting for leader to pick prompt...</p>
          </div>
        )}

        {room.status === 'TOPIC_REVEAL' && (
          <div className="space-y-12 animate-in zoom-in duration-500 w-full max-w-5xl overflow-hidden leading-none uppercase">
            <div className="space-y-4 leading-none uppercase uppercase"><p className="text-yellow-400 font-black uppercase tracking-[0.3em] text-xs italic leading-none leading-none uppercase uppercase">The Controversy</p><h2 className="text-5xl md:text-8xl font-black italic leading-tight text-white drop-shadow-lg break-words leading-none leading-none uppercase uppercase uppercase uppercase">"{room.topic}"</h2></div>
            <div className="flex justify-center gap-8 md:gap-16 items-center flex-wrap leading-none uppercase uppercase uppercase">
              <div className="space-y-2 uppercase uppercase uppercase"><p className="text-indigo-400 uppercase font-black text-[10px] tracking-widest leading-none leading-none uppercase uppercase uppercase">Active Player</p><div className="text-3xl md:text-5xl font-black text-white uppercase leading-none leading-none uppercase uppercase uppercase">{speaker?.name}</div></div>
              {room.roundType === 3 && (<><Swords className="w-8 h-8 md:w-12 md:h-12 text-pink-500 animate-pulse leading-none leading-none uppercase uppercase uppercase" /><div className="space-y-2 uppercase uppercase uppercase"><p className="text-indigo-400 uppercase font-black text-[10px] tracking-widest leading-none leading-none uppercase uppercase uppercase">Opponent</p><div className="text-3xl md:text-5xl font-black text-white uppercase leading-none leading-none uppercase uppercase uppercase uppercase">{opponent?.name}</div></div></>)}
            </div>
            <div className="flex flex-col items-center gap-6 leading-none uppercase uppercase uppercase">
               {room.roundType === 2 && !room.sneakyWord ? (<div className="bg-emerald-500/10 border-2 border-emerald-500/30 p-8 rounded-[2rem] animate-pulse leading-none leading-none uppercase uppercase uppercase"><p className="text-emerald-400 font-black uppercase text-xl italic leading-none leading-none uppercase uppercase uppercase uppercase">The Plant is Sabotaging...</p></div>) : (<div className="relative group leading-none leading-none uppercase uppercase uppercase"><div className="absolute inset-0 bg-yellow-400 blur-2xl opacity-10 animate-pulse leading-none leading-none uppercase uppercase uppercase"></div><div className="relative bg-indigo-900/50 px-12 py-8 rounded-[3rem] border-2 border-indigo-700 flex flex-col items-center leading-none leading-none uppercase uppercase uppercase"><p className="text-indigo-400 font-black uppercase text-[10px] mb-2 tracking-[0.3em] leading-none leading-none uppercase uppercase uppercase">Preparation Clock</p><p className="text-8xl font-black text-white tabular-nums drop-shadow-glow leading-none leading-none uppercase uppercase uppercase uppercase">{room.prepCountdown}</p></div></div>)}
            </div>
          </div>
        )}

        {room.status === 'SPEAKING' && (
          <div className="space-y-12 w-full max-w-6xl animate-in fade-in leading-none uppercase uppercase uppercase">
            <div className={`text-6xl md:text-9xl font-black italic mb-20 drop-shadow-2xl break-words px-4 leading-none leading-none uppercase uppercase uppercase ${room.isSecondHalf ? 'text-pink-500 scale-105' : 'text-white'}`}>{room.roundType === 3 && !room.isSecondHalf ? `${speaker?.name}` : room.roundType === 3 ? `${opponent?.name}` : room.topic}</div>
            <div className="flex flex-col items-center gap-12 leading-none uppercase uppercase uppercase">
              <div className="relative w-56 h-56 md:w-72 md:h-72 flex items-center justify-center leading-none leading-none uppercase uppercase uppercase"><div className="absolute inset-0 border-8 border-indigo-800 rounded-full leading-none uppercase uppercase uppercase"></div><div className={`absolute inset-0 border-8 border-yellow-400 rounded-full animate-ping opacity-20 leading-none leading-none uppercase uppercase uppercase`} style={{ animationDuration: '2s' }}></div><Mic2 className="w-16 h-16 md:w-24 md:h-24 text-yellow-400 drop-shadow-glow leading-none leading-none uppercase uppercase uppercase" /></div>
              <div className="text-3xl md:text-4xl font-black uppercase text-indigo-400 tracking-widest h-20 leading-none leading-none uppercase uppercase uppercase">{room.roundType === 3 && speakTime >= 14 && speakTime <= 16 ? <span className="text-pink-500 animate-bounce block text-8xl uppercase uppercase uppercase uppercase">SWITCH!</span> : "Internal Clock Active"}</div>
            </div>
          </div>
        )}

        {activeHeckle && (
          <div key={activeHeckle.id} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-in zoom-in slide-in-from-bottom-20 pointer-events-none z-50 w-full max-w-2xl text-center leading-none uppercase uppercase uppercase">
            <div className="bg-red-500 p-8 md:p-12 rounded-[3rem] text-white font-black text-5xl md:text-8xl shadow-2xl border-8 border-white/20 uppercase italic break-words mx-4 leading-none leading-none uppercase uppercase uppercase uppercase">{activeHeckle.text}</div>
            <p className="mt-6 text-2xl font-bold bg-indigo-950 px-6 py-3 rounded-2xl border-2 border-white/20 shadow-xl inline-block leading-none leading-none uppercase uppercase uppercase uppercase">{activeHeckle.sender}</p>
          </div>
        )}

        {room.status === 'VOTING_WORD' && (
           <div className="space-y-12 animate-in zoom-in w-full max-w-4xl leading-none uppercase uppercase uppercase">
              <h2 className="text-7xl font-black uppercase italic tracking-tighter text-emerald-400 leading-none leading-none uppercase uppercase uppercase">Sneaky Check!</h2>
              <div className="bg-indigo-900 p-12 rounded-[3rem] border-4 border-emerald-600 shadow-2xl space-y-6 leading-none leading-none uppercase uppercase uppercase uppercase uppercase"><p className="text-indigo-300 uppercase font-black tracking-widest leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase">Secret Word Was:</p><h3 className="text-8xl font-black text-white uppercase italic tracking-tighter leading-none leading-none uppercase uppercase uppercase uppercase uppercase">"{room.sneakyWord}"</h3><p className="text-2xl text-white leading-none uppercase uppercase uppercase">Audience: Did they slip it in naturally?</p></div>
              <div className="flex justify-center gap-12 leading-none uppercase uppercase uppercase">
                 <div className="text-center leading-none leading-none uppercase uppercase uppercase uppercase"><p className="text-8xl font-black text-emerald-400 leading-none leading-none uppercase uppercase uppercase uppercase">Yes</p></div>
                 <div className="text-center leading-none leading-none uppercase uppercase uppercase uppercase"><p className="text-8xl font-black text-red-400 leading-none leading-none uppercase uppercase uppercase uppercase">No</p></div>
              </div>
           </div>
        )}
        ...
        {room.status === 'RESULTS' && (
          <div className="space-y-12 animate-in zoom-in w-full overflow-hidden text-center leading-none uppercase uppercase uppercase">
             <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter text-white leading-none leading-none uppercase uppercase uppercase uppercase">The Scoring</h2>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto px-4 text-center leading-none uppercase uppercase uppercase uppercase">
                <div className="bg-indigo-900/50 p-10 md:p-16 rounded-[3rem] border-2 border-white/10 shadow-2xl overflow-hidden text-center leading-none uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-indigo-400 font-black uppercase text-xl md:text-2xl tracking-widest mb-4 leading-none uppercase uppercase uppercase uppercase uppercase uppercase">Final Time</p><p className="text-[6rem] md:text-[10rem] font-black text-yellow-400 leading-none uppercase uppercase uppercase uppercase uppercase">{(speaker?.lastTurnTime || 0).toFixed(2)}s</p></div>
                <div className="bg-indigo-900/50 p-10 md:p-16 rounded-[3rem] border-2 border-white/10 shadow-2xl overflow-hidden text-center leading-none uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-indigo-400 font-black uppercase text-xl md:text-2xl tracking-widest mb-4 leading-none uppercase uppercase uppercase uppercase uppercase uppercase">Points Gained</p><p className="text-[6rem] md:text-[10rem] font-black text-white leading-none uppercase uppercase uppercase uppercase uppercase">+{speaker?.lastTurnScore || 0}</p></div>
             </div>
             <p className="text-2xl font-black uppercase text-yellow-400 italic animate-pulse leading-none mt-8 uppercase leading-none uppercase uppercase uppercase uppercase">Speaker controls the show...</p>
          </div>
        )}

        {room.status === 'FINAL_PODIUM' && (
           <div className="space-y-12 animate-in slide-in-from-bottom-20 duration-1000 w-full max-w-4xl px-4 overflow-hidden relative text-center leading-none uppercase uppercase uppercase uppercase">
              <Confetti />
              ...
              <div className="grid gap-4 mt-12 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide leading-none uppercase uppercase uppercase uppercase uppercase uppercase">{sortedWinners.slice(0, 3).map((p, i) => (<div key={p.uid} className={`flex items-center justify-between p-6 md:p-8 rounded-[2.5rem] border-4 backdrop-blur-md shadow-2xl ${i === 0 ? 'bg-yellow-400 text-indigo-950 border-white/50 scale-105' : 'bg-indigo-900 text-white border-indigo-700 opacity-80'} leading-none uppercase uppercase uppercase uppercase uppercase uppercase`}><div className="flex items-center gap-6 leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><span className="text-4xl md:text-6xl font-black italic opacity-40 leading-none uppercase uppercase uppercase uppercase uppercase">#{i+1}</span><span className="text-3xl md:text-5xl font-black uppercase tracking-tighter truncate max-w-[200px] leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">{p.name}</span></div><div className="text-right leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase"><span className="text-3xl md:text-5xl font-black leading-none uppercase uppercase uppercase uppercase uppercase uppercase">{p.score}</span><span className="text-[10px] uppercase font-black block tracking-widest leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Points</span></div></div>))}</div>
      ...
      <div className="bg-indigo-900 p-6 md:p-8 flex justify-center flex-wrap gap-4 md:gap-8 border-t-4 border-black/20 shadow-2xl max-h-[140px] overflow-hidden shrink-0 leading-none uppercase uppercase uppercase uppercase">
        {players.sort((a,b) => b.score - a.score).map((p, i) => (<div key={p.uid} className="flex items-center gap-4 bg-indigo-950/50 px-5 md:px-6 py-3 rounded-2xl border-2 border-indigo-700 shadow-inner group transition-all leading-none uppercase uppercase uppercase uppercase uppercase"><div className="font-black text-indigo-500 text-2xl italic group-hover:text-yellow-400 leading-none uppercase uppercase uppercase uppercase uppercase uppercase">#{i+1}</div><div className="text-left leading-tight leading-none uppercase uppercase uppercase uppercase uppercase uppercase"><p className="font-black uppercase text-[10px] md:text-xs tracking-tighter truncate max-w-[80px] text-indigo-100 uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase">{p.name}</p><p className="font-black text-yellow-400 text-xl md:text-2xl leading-none mt-1 leading-none uppercase uppercase uppercase uppercase uppercase uppercase">{p.score}</p></div></div>))}
      </div>
    </div>
  );
}
...
function PlayerView({ room, players, user, stopSpeaking, setupTurn, advanceGame, startNextRound, joinRoom, playerName, setPlayerName, roomCodeInput, setRoomCodeInput }) {
  ...
  if (!me) {
    return (
      <div className="min-h-screen bg-indigo-950 text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden uppercase leading-none uppercase uppercase uppercase uppercase">
        <div className="max-w-md w-full text-center space-y-12 animate-in fade-in zoom-in duration-500 uppercase leading-none uppercase uppercase uppercase uppercase uppercase">
          <div className="space-y-4 leading-none uppercase uppercase uppercase uppercase"><h1 className="text-6xl md:text-7xl font-black italic text-yellow-400 uppercase tracking-tighter leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase">HERE'S MY POINT!</h1><p className="text-indigo-400 font-bold uppercase text-xs tracking-[0.4em] leading-none uppercase uppercase uppercase uppercase uppercase">The Ultimate Debate Party</p></div>
          <div className="bg-indigo-900/40 p-8 rounded-[2.5rem] border-2 border-indigo-800 space-y-6 text-left shadow-xl uppercase leading-none uppercase uppercase uppercase uppercase">
            <div className="flex items-center gap-2 text-indigo-300 relative z-10 leading-none uppercase uppercase uppercase uppercase uppercase"><Smartphone className="w-5 h-5 leading-none uppercase uppercase uppercase uppercase uppercase uppercase" /><p className="font-black uppercase text-[10px] tracking-[0.2em] leading-none uppercase uppercase uppercase uppercase uppercase">Join the Party</p></div>
            <form onSubmit={e => joinRoom(e)} className="space-y-4 relative z-10 leading-none uppercase uppercase uppercase uppercase uppercase">
              <input type="text" placeholder="CODE" maxLength={4} className="w-full bg-indigo-950 text-center font-black text-5xl py-4 rounded-3xl uppercase text-white border-2 border-indigo-800 shadow-inner focus:outline-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase" value={roomCodeInput} onChange={e => setRoomCodeInput(e.target.value)} />
              <input type="text" placeholder="YOUR NAME" className="w-full bg-indigo-900 p-5 rounded-2xl font-bold border-2 border-indigo-800 focus:outline-none focus:border-indigo-400 text-indigo-100 uppercase leading-none uppercase uppercase uppercase uppercase uppercase" value={playerName} onChange={e => setPlayerName(e.target.value)} />
              <button type="submit" className="w-full bg-yellow-400 text-indigo-950 py-6 rounded-[2rem] font-black text-2xl uppercase shadow-lg leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase">ENTER LOBBY</button>
            </form>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-[100dvh] bg-indigo-950 text-white flex flex-col font-sans touch-none select-none overflow-hidden max-w-full uppercase leading-none uppercase leading-none uppercase uppercase uppercase uppercase">
       <div className="p-4 bg-indigo-900 flex justify-between items-center border-b-2 border-black/20 shadow-md shrink-0 leading-none uppercase leading-none uppercase uppercase uppercase uppercase">
          ...
          <div className="flex gap-4 leading-none uppercase uppercase uppercase uppercase uppercase">
             ...
             <div className="text-right leading-none leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-[10px] font-black text-indigo-400 mb-1 leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Score</p><p className="text-lg font-black text-yellow-400 tabular-nums leading-none leading-none uppercase uppercase uppercase uppercase uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase">{me?.score || 0}</p></div>
          </div>
       </div>
       ...
       <div className="flex-1 flex flex-col p-6 overflow-y-auto leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase">
            ...
            <div className="flex-1 flex flex-col max-w-full leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
              {(isSpeaker || isOpponent) ? (
                <div className="flex-1 flex flex-col items-center justify-center space-y-12 leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
                   <div className="text-center space-y-2 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-yellow-400 font-black uppercase text-[10px] tracking-widest leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Mic is Active</p><h2 className="text-5xl font-black italic uppercase tracking-tighter leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">{(isOpponent && !room.isSecondHalf) ? 'ON DECK...' : (isSpeaker && room.isSecondHalf) ? 'WAIT...' : 'DEFEND!'}</h2></div>
                   <div className="w-64 h-64 rounded-full border-[16px] border-indigo-900 flex items-center justify-center relative bg-indigo-950 shadow-inner leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">{(isOpponent && room.isSecondHalf) || (isSpeaker && !room.isSecondHalf) ? <div className="absolute inset-0 rounded-full border-8 border-yellow-400 animate-ping opacity-10 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"></div> : null}<Mic2 className={`w-20 h-20 text-yellow-400 ${((isSpeaker && !room.isSecondHalf) || (isOpponent && room.isSecondHalf)) ? 'opacity-100 animate-pulse' : 'opacity-10'} leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase`} /></div>
                   {((room.roundType !== 3 && isSpeaker) || (room.roundType === 3 && isOpponent && room.isSecondHalf)) && (<button onClick={() => stopSpeaking((Date.now() - room.startTime) / 1000)} className="w-full bg-red-500 py-12 rounded-[3rem] font-black text-5xl uppercase tracking-tighter shadow-[0_12px_0_rgb(150,0,0)] active:translate-y-3 active:shadow-none transition-all leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">STOP!</button>)}
                </div>
              ) : (
                <div className="flex-1 flex flex-col space-y-6 max-w-full overflow-hidden leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
                   <div className="bg-indigo-900/50 p-4 rounded-[2rem] border border-white/5 text-center leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-[10px] font-black text-indigo-400 tracking-widest mb-1 leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Currently Active</p><h3 className="text-xs font-black italic truncate leading-none opacity-50 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">"{room.topic}"</h3></div>
                   <form onSubmit={e => { e.preventDefault(); if (!customHeckle || (me?.hecklesLeft || 0) <= 0) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code, 'players', user.uid), { hecklesLeft: increment(-1) }); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { lastHeckle: { id: Math.random().toString(), text: customHeckle.toUpperCase(), sender: me.name } }); setCustomHeckle(''); }} className="space-y-4 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
                      <div className="text-center space-y-2 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase leading-none uppercase uppercase uppercase uppercase"><p className="text-[10px] font-black text-red-500 tracking-widest leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase leading-none uppercase">Distraction Engine</p><input type="text" placeholder="TYPE A HECKLE..." maxLength={HECKLE_CHAR_LIMIT} className="w-full bg-indigo-900 p-6 rounded-[2rem] border-4 border-indigo-800 font-black text-2xl text-center focus:border-red-500 text-white outline-none leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase" value={customHeckle} onChange={e => setCustomHeckle(e.target.value)} /></div>
                      <button type="submit" disabled={(me?.hecklesLeft || 0) <= 0 || !customHeckle} className="w-full bg-red-500 text-white p-7 rounded-[2rem] border-b-8 border-red-900 font-black uppercase text-3xl flex items-center justify-center gap-4 active:translate-y-2 active:border-b-0 shadow-xl disabled:opacity-10 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">FIRE!</button>
                   </form>
                   <div className="mt-auto flex justify-center items-center gap-1 leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><p className="text-[10px] font-black text-indigo-400 tracking-widest mr-1 leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Ammo:</p>{[...Array(MAX_HECKLES)].map((_, i) => (<div key={i} className={`w-2 h-4 rounded-full transition-all duration-300 ${i < (me?.hecklesLeft || 0) ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-indigo-950 opacity-20'} leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase`} />))}</div>
                </div>
              )}
            </div>
          )}
          ...
          {room.status === 'VOTING_WORD' && (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
                <h2 className="text-4xl font-black uppercase text-emerald-400 italic tracking-tighter leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Word Check!</h2>
                ...
                {!isSpeaker ? (
                  <div className="grid grid-cols-2 gap-4 w-full leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><button onClick={() => castVote('YES')} className="bg-emerald-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><ThumbsUp className="w-12 h-12 mx-auto leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase" /></button><button onClick={() => castVote('NO')} className="bg-red-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><ThumbsDown className="w-12 h-12 mx-auto leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase" /></button></div>
                ) : (
                   <p className="text-indigo-400 italic uppercase uppercase uppercase">Wait for verdict...</p>
                )}
                {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 leading-none uppercase leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">TALLY VOTES</button>}
             </div>
          )}

          {room.status === 'VOTING_VIBE' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">
               <h2 className="text-4xl font-black uppercase text-pink-500 italic tracking-tighter leading-none text-center leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Vibe Check!</h2>
               <button onClick={() => castVote(room.currentSpeakerUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">{players.find(p => p.uid === room.currentSpeakerUid)?.name}</button>
               ...
               {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 mt-4 leading-none uppercase leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Award Bonus</button>}
            </div>
          )}

          {(room.status === 'RESULTS' || room.status === 'FINAL_PODIUM') && <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in zoom-in leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase"><Trophy className="w-24 h-24 text-yellow-400 animate-bounce leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase" /><h2 className="text-5xl font-black italic tracking-tighter text-center leading-none leading-none leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Turn<br/>Finished</h2>{(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-yellow-400 text-indigo-950 py-10 rounded-[3rem] font-black text-4xl shadow-2xl active:scale-95 animate-pulse leading-none uppercase leading-none uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase uppercase">Continue Show</button>}</div >}
       </div>
       <style>{`

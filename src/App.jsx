...
export default function App() {
  ...
  // Room Listener - Fixed to prevent leaks and infinite heckle loops
  useEffect(() => {
    ...
    const unsubRoom = onSnapshot(roomRef, (snap) => {
      if (snap.exists()) {
        ...
        // Fixed Heckle Logic: Use Ref to prevent state-dependency loops
  ...
  }, [roomCode, user, role]); // Only re-subscribe if major state changes

  // Lobby Music Controller
  useEffect(() => {
    if (role === 'host' && room?.status === 'LOBBY') {
      if (!introAudioRef.current) {
        introAudioRef.current = new Audio(`/sounds/intro.${AUDIO_EXT}`);
  ...
  const advanceGame = async () => {
    ...
    if (nextIdx >= limit) {
      ...
      if (room.roundType < 3) {
        ...
        if (room.roundType === 2) turnData.plantUid = players.filter(p => p.uid !== turnData.currentSpeakerUid)[0].uid;
      } else {
        turnData.currentSpeakerUid = room.roundOrder[nextIdx * 2];
        const secondId = room.roundOrder[nextIdx * 2 + 1];
        if (secondId) { turnData.opponentUid = secondId; } 
  ...
  const stopSpeaking = async (duration) => {
    ...
    // Corrected score path to use increment for atomicity
  ...
  // --- Screens ---
  if (authLoading) return <div className="min-h-screen bg-indigo-950 flex flex-col items-center justify-center p-8 text-white"><Loader2 className="animate-spin mb-4" /><p className="uppercase text-xs font-bold tracking-widest">Waking Up...</p></div>;
    ...
    return (
      <div className="min-h-screen bg-indigo-950 text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden">
        <div className="max-w-md w-full text-center space-y-12 animate-in fade-in zoom-in duration-500">
          <div className="space-y-4"><h1 className="text-6xl md:text-7xl font-black italic text-yellow-400 uppercase tracking-tighter transform -rotate-2 leading-none">HERE'S MY POINT!</h1><p className="text-indigo-400 font-bold uppercase text-xs tracking-[0.4em]">The Ultimate Debate Party</p></div>
          <div className="grid gap-6">
            <button onClick={createRoom} className="group bg-indigo-600 hover:bg-indigo-500 p-8 rounded-[2.5rem] flex items-center justify-between border-b-8 border-indigo-900 shadow-2xl transition-all active:translate-y-2 active:border-b-0"><div className="text-left"><p className="font-black text-2xl uppercase italic leading-none mb-1 text-white">Host Show</p><p className="text-indigo-300 text-sm font-bold">Project on Big Screen</p></div><Monitor className="w-12 h-12 text-indigo-300" /></button>
          ...
          {error && <div className="bg-red-500/20 border border-red-500/50 p-4 rounded-2xl text-red-200 font-bold flex items-center gap-2 animate-bounce"><AlertTriangle className="w-5 h-5 shrink-0" /><p className="text-xs text-left">{error}</p></div>}
        </div>
      </div>
    );
  }
...
// --- Host Components ---
function HostView({ room, players, roomCode, setupTurn, startSpeaking, activeHeckle, restartGame }) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative text-center overflow-hidden">
        ...
        {room.status === 'TOPIC_REVEAL' && (
          <div className="space-y-12 animate-in zoom-in w-full max-w-5xl">
            <div className="space-y-4"><p className="text-yellow-400 font-black uppercase tracking-[0.3em] text-xs italic">The Controversy</p><h2 className="text-5xl md:text-8xl font-black italic leading-tight text-white drop-shadow-lg break-words leading-none">"{room.topic}"</h2></div>
            <div className="flex justify-center gap-8 md:gap-16 items-center flex-wrap">
              <div className="space-y-2"><p className="text-indigo-400 uppercase font-black text-[10px] tracking-widest leading-none">Active Player</p><div className="text-3xl md:text-5xl font-black text-white leading-none">{speaker?.name}</div></div>
              {room.roundType === 3 && (<><Swords className="w-8 h-8 md:w-12 md:h-12 text-pink-500 animate-pulse" /><div className="space-y-2"><p className="text-indigo-400 uppercase font-black text-[10px] tracking-widest leading-none">Opponent</p><div className="text-3xl md:text-5xl font-black text-white leading-none">{opponent?.name}</div></div></>)}
            </div>
            <div className="flex flex-col items-center gap-6">
               {room.roundType === 2 && !room.sneakyWord ? (<div className="bg-emerald-500/10 border-2 border-emerald-500/30 p-8 rounded-[2rem] animate-pulse"><p className="text-emerald-400 font-black uppercase text-xl italic leading-none">The Plant is Sabotaging...</p></div>) : (<div className="relative group"><div className="absolute inset-0 bg-yellow-400 blur-2xl opacity-10 animate-pulse"></div><div className="relative bg-indigo-900/50 px-12 py-8 rounded-[3rem] border-2 border-indigo-700 flex flex-col items-center"><p className="text-indigo-400 font-black uppercase text-[10px] mb-2 tracking-[0.3em] leading-none">Preparation Clock</p><p className="text-8xl font-black text-white tabular-nums drop-shadow-glow leading-none">{room.prepCountdown}</p></div></div>)}
            </div>
          </div>
        )}
        ...
        {room.status === 'VOTING_WORD' && (
           <div className="space-y-12 animate-in zoom-in w-full max-w-4xl">
              ...
              <div className="bg-indigo-900 p-12 rounded-[3rem] border-4 border-emerald-600 shadow-2xl space-y-6"><p className="text-indigo-300 uppercase font-black tracking-widest leading-none uppercase">Secret Word Was:</p><h3 className="text-8xl font-black text-white uppercase italic tracking-tighter leading-none">"{room.sneakyWord}"</h3><p className="text-2xl text-white">Audience: Did they slip it in naturally?</p></div>
        ...
        {room.status === 'INTERROGATION' && (
          <div className="space-y-12 animate-in zoom-in w-full max-w-5xl">
            <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter text-indigo-400 leading-none">Interrogation!</h2>
            <div className="bg-white text-indigo-950 p-12 rounded-[4rem] shadow-2xl border-b-[16px] border-indigo-200">
               ...
               <h3 className="text-6xl font-black italic leading-tight uppercase leading-none">"{room.chosenQuestion?.text}"</h3>
        ...
        {room.status === 'VOTING_VIBE' && (
          <div className="space-y-12 animate-in zoom-in w-full max-w-5xl">
            <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter text-pink-400 leading-none">Vibe Check!</h2>
        ...
        {room.status === 'RESULTS' && (
          <div className="space-y-12 animate-in zoom-in w-full overflow-hidden text-center">
             <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter text-white">The Scoring</h2>
        ...
        {room.status === 'FINAL_PODIUM' && (
           <div className="space-y-12 animate-in slide-in-from-bottom-20 duration-1000 w-full max-w-4xl px-4 overflow-hidden relative text-center">
              ...
              <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter text-white leading-none">The Winners</h2>
              <div className="grid gap-4 mt-12 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">{sortedWinners.slice(0, 3).map((p, i) => (<div key={p.uid} className={`flex items-center justify-between p-6 md:p-8 rounded-[2.5rem] border-4 backdrop-blur-md shadow-2xl ${i === 0 ? 'bg-yellow-400 text-indigo-950 border-white/50 scale-105' : 'bg-indigo-900 text-white border-indigo-700 opacity-80'}`}><div className="flex items-center gap-6 leading-none"><span className="text-4xl md:text-6xl font-black italic opacity-40 leading-none">#{i+1}</span><span className="text-3xl md:text-5xl font-black uppercase tracking-tighter truncate max-w-[200px] leading-none">{p.name}</span></div><div className="text-right leading-none"><span className="text-3xl md:text-5xl font-black leading-none">{p.score}</span><span className="text-[10px] uppercase font-black block tracking-widest leading-none">Points</span></div></div>))}</div>
              <button onClick={restartGame} className="bg-indigo-600 px-12 py-5 rounded-2xl font-black text-xl md:text-2xl uppercase flex items-center gap-3 shadow-xl active:scale-95 mx-auto mt-8 leading-none"><RotateCcw className="w-6 h-6" /> Replay Show</button>
           </div>
        )}
      </div>
...
function PlayerView({ room, players, user, stopSpeaking, setupTurn, advanceGame, startNextRound }) {
  ...
  if (!room || !me) return <div className="min-h-screen bg-indigo-950 flex items-center justify-center p-8"><Loader2 className="w-16 h-16 text-yellow-400 animate-spin" /></div>;
       ...
       <div className="flex-1 flex flex-col p-6 overflow-y-auto">
          {room.status === 'LOBBY' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
              ...
              <h2 className="text-4xl font-black uppercase italic tracking-tighter leading-none text-white leading-none">HERE'S MY POINT!</h2>
            ...
            <div className="flex-1 flex flex-col items-center justify-center space-y-10 text-center max-w-full">
              {(isSpeaker || (isOpponent && room.roundType === 3)) ? (
                ...
                <>
                  ...
                  <button onClick={() => { if (!sneakyInput) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { sneakyWord: sneakyInput.toUpperCase() }); setSneakyInput(''); }} className="w-full bg-emerald-500 text-indigo-950 py-6 rounded-[2rem] font-black text-2xl uppercase shadow-lg active:scale-95 leading-none uppercase">Sabotage!</button>
                </>
            ...
            <div className="flex-1 flex flex-col max-w-full">
              {(isSpeaker || isOpponent) ? (
                <div className="flex-1 flex flex-col items-center justify-center space-y-12">
                   <div className="text-center space-y-2 leading-none"><p className="text-yellow-400 font-black uppercase text-[10px] tracking-widest leading-none uppercase">Mic is Active</p><h2 className="text-5xl font-black italic uppercase tracking-tighter leading-none">{(isOpponent && !room.isSecondHalf) ? 'ON DECK...' : (isSpeaker && room.isSecondHalf) ? 'WAIT...' : 'DEFEND!'}</h2></div>
                   <div className="w-64 h-64 rounded-full border-[16px] border-indigo-900 flex items-center justify-center relative bg-indigo-950 shadow-inner leading-none">{(isOpponent && room.isSecondHalf) || (isSpeaker && !room.isSecondHalf) ? <div className="absolute inset-0 rounded-full border-8 border-yellow-400 animate-ping opacity-10"></div> : null}<Mic2 className={`w-20 h-20 text-yellow-400 ${((isSpeaker && !room.isSecondHalf) || (isOpponent && room.isSecondHalf)) ? 'opacity-100 animate-pulse' : 'opacity-10'}`} /></div>
                   {((room.roundType !== 3 && isSpeaker) || (room.roundType === 3 && isOpponent && room.isSecondHalf)) && (<button onClick={() => stopSpeaking((Date.now() - room.startTime) / 1000)} className="w-full bg-red-500 py-12 rounded-[3rem] font-black text-5xl uppercase tracking-tighter shadow-[0_12px_0_rgb(150,0,0)] active:translate-y-3 active:shadow-none transition-all leading-none uppercase">STOP!</button>)}
                </div>
              ) : (
                <div className="flex-1 flex flex-col space-y-6 max-w-full overflow-hidden leading-none">
                   <div className="bg-indigo-900/50 p-4 rounded-[2rem] border border-white/5 text-center leading-none"><p className="text-[10px] font-black text-indigo-400 tracking-widest mb-1 leading-none uppercase">Currently Active</p><h3 className="text-xs font-black italic truncate leading-none opacity-50 leading-none">"{room.topic}"</h3></div>
                   {room.roundType === 2 && (<form onSubmit={(e) => { e.preventDefault(); if (!questionInput) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { questions: [...(room.questions || []), { text: questionInput.toUpperCase(), sender: me.name, id: Math.random().toString() }] }); setQuestionInput(''); }} className="bg-indigo-800/40 p-5 rounded-3xl border border-indigo-600/50 space-y-3 leading-none"><p className="text-[10px] font-black text-indigo-300 tracking-widest text-center leading-none">Interrogation Lab</p><input type="text" placeholder="ASK A QUESTION..." maxLength={50} className="w-full bg-indigo-950 p-4 rounded-2xl border-2 border-indigo-700 outline-none text-sm text-center leading-none" value={questionInput} onChange={e => setQuestionInput(e.target.value)} /><button type="submit" disabled={!questionInput} className="w-full bg-indigo-400 text-indigo-950 py-3 rounded-2xl font-black text-xs leading-none uppercase">SUBMIT QUESTION</button></form>)}
                   <form onSubmit={(e) => { e.preventDefault(); if (!customHeckle || (me?.hecklesLeft || 0) <= 0) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code, 'players', user.uid), { hecklesLeft: increment(-1) }); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', room.code), { lastHeckle: { id: Math.random().toString(), text: customHeckle.toUpperCase(), sender: me.name } }); setCustomHeckle(''); }} className="space-y-4 leading-none">
                      <div className="text-center space-y-2 leading-none"><p className="text-[10px] font-black text-red-500 tracking-widest leading-none">Distraction Engine</p><input type="text" placeholder="TYPE A HECKLE..." maxLength={HECKLE_CHAR_LIMIT} className="w-full bg-indigo-900 p-6 rounded-[2rem] border-4 border-indigo-800 font-black text-2xl text-center focus:border-red-500 text-white outline-none leading-none" value={customHeckle} onChange={e => setCustomHeckle(e.target.value)} /></div>
                   ...
                   <div className="mt-auto flex justify-center items-center gap-1 leading-none"><p className="text-[10px] font-black text-indigo-400 tracking-widest mr-1 leading-none uppercase">Ammo:</p>{[...Array(MAX_HECKLES)].map((_, i) => (<div key={i} className={`w-2 h-4 rounded-full transition-all duration-300 ${i < (me?.hecklesLeft || 0) ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-indigo-950 opacity-20'}`} />))}</div>
                </div>
              )}
            </div>
          )}
          ...
          {room.status === 'VOTING_WORD' && (
             <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none">
                <h2 className="text-4xl font-black uppercase text-emerald-400 italic tracking-tighter leading-none">Word Check!</h2>
                <div className="bg-indigo-900 p-8 rounded-3xl border-2 border-emerald-500/30 w-full text-center font-black text-3xl text-white italic leading-none uppercase">"{room.sneakyWord}"</div>
                <div className="grid grid-cols-2 gap-4 w-full"><button onClick={() => castVote('YES')} className="bg-emerald-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none"><ThumbsUp className="w-12 h-12 mx-auto" /></button><button onClick={() => castVote('NO')} className="bg-red-500 p-10 rounded-[2rem] font-black text-3xl active:scale-95 shadow-xl leading-none"><ThumbsDown className="w-12 h-12 mx-auto" /></button></div>
                {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 leading-none uppercase">TALLY VOTES</button>}
             </div>
          )}

          {room.status === 'INTERROGATION' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none">
               <h2 className="text-4xl font-black uppercase text-indigo-400 italic tracking-tighter leading-none text-center leading-none">Verdict!</h2>
               <button onClick={() => castVote('SMOOTH')} className="w-full bg-indigo-500 p-8 rounded-[2.5rem] border-4 border-indigo-400 font-black text-2xl active:bg-yellow-400 shadow-xl leading-none leading-none">SMOOTH (+200)</button>
               <button onClick={() => castVote('RATTLED')} className="w-full bg-red-900 p-8 rounded-[2.5rem] border-4 border-red-800 font-black text-2xl active:bg-red-500 shadow-xl leading-none leading-none">RATTLED</button>
               {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 mt-4 leading-none uppercase">Finish Verdict</button>}
            </div>
          )}
          ...
          {room.status === 'VOTING_VIBE' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in zoom-in leading-none">
               ...
               <button onClick={() => castVote(room.currentSpeakerUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none">{players.find(p => p.uid === room.currentSpeakerUid)?.name}</button>
               <button onClick={() => castVote(room.opponentUid)} className="w-full bg-indigo-900 p-8 rounded-[2.5rem] border-4 border-indigo-800 font-black text-2xl active:bg-yellow-400 transition-all shadow-xl uppercase leading-none">{players.find(p => p.uid === room.opponentUid)?.name}</button>
               {(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-white text-indigo-950 py-6 rounded-[2rem] font-black text-xl active:scale-95 mt-4 leading-none uppercase">Award Bonus</button>}
            </div>
          )}

          {room.status === 'RESULTS' && <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in zoom-in leading-none"><Trophy className="w-24 h-24 text-yellow-400 animate-bounce" /><h2 className="text-5xl font-black italic tracking-tighter text-center leading-none leading-none">Turn<br/>Finished</h2>{(isSpeaker || isOpponent || isLeader) && <button onClick={handleContinue} className="w-full bg-yellow-400 text-indigo-950 py-10 rounded-[3rem] font-black text-4xl shadow-2xl active:scale-95 animate-pulse leading-none uppercase">Continue Show</button>}</div >}
       </div>
       <style>{`
